...
public extension Hashing {
    ...
    struct SHA384: HashFunction {
        /// The number of bytes in a SHA384 digest.
        public static var byteCount: Int { Int(CC_SHA384_DIGEST_LENGTH) }

        /// The block size in bytes for a SHA384digest.
        public static var blockByteCount: Int { Int(CC_SHA384_BLOCK_BYTES) }
        ...
        public static func hash(_ source: some DataProtocol) -> Data {
            ...
            digest.withUnsafeMutableBytes { (digestPointer: UnsafeMutableRawBufferPointer) in
                data.withUnsafeBytes { (dataPointer: UnsafeRawBufferPointer) in
                    _ = CC_SHA384(dataPointer.baseAddress!, dataLength, digestPointer.bindMemory(to: UInt8.self).baseAddress!)
                }
            }
            return digest
        }
        ...
        public init() {
            withUnsafeMutablePointer(to: &hashContext) { _ = CC_SHA384_Init($0) }
        }
        ...
        public mutating func update(data: some DataProtocol) {
            ...
            withUnsafeMutablePointer(to: &hashContext) { hashContextPointer in
                dataToHash.withUnsafeBytes { (dataToHashPointer: UnsafeRawBufferPointer) in
                    _ = CC_SHA384_Update(hashContextPointer, dataToHashPointer.baseAddress!, dataLength)
                }
            }
        }
        ...
        public mutating func finalize() -> Data {
            ...
            withUnsafeMutablePointer(to: &hashContext) { hashContextPointer in
                digest.withUnsafeMutableBytes { (digestPointer: UnsafeMutableRawBufferPointer) in
                    _ = CC_SHA384_Final(digestPointer.bindMemory(to: UInt8.self).baseAddress, hashContextPointer)
                }
            }
            return digest
        }
